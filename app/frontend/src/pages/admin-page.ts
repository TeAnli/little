      ...
      <div class="page-enter">
        <div class="flex items-center justify-between mb-10">
          ...
          <div class="flex gap-2">
            <button class="btn-primary micro-lift px-4 py-2 text-sm"
              @click=${() => navigate('/editor')}>写文章</button>
            <button class="btn-ghost micro-lift px-4 py-2 text-sm text-muted"
              @click=${logout}>退出</button>
          </div>
        </div>

        ${this.loading ? html`<div class="py-6"><div class="shimmer h-4 rounded w-1/2"></div></div>`
        : this.posts.length === 0 ? html`<div class="py-16 text-center text-muted">暂无文章</div>`
          ...
          <div class="flex flex-col border-t hairline">
            ${this.posts.map(p => html`
              <div class="hover-bg-scale flex items-center justify-between gap-4 py-4 px-3 -mx-3 rounded-[var(--radius-box)] border-b hairline">
                ...
                <div class="flex gap-1 shrink-0">
                  <button class="btn-ghost micro-lift px-3 py-1.5 text-xs text-fg"
                    @click=${() => navigate('/editor/' + p.slug)}>编辑</button>
                  <button class="btn-ghost micro-lift px-3 py-1.5 text-xs text-red-500"
        ...
        ${this._pages > 1 ? html`
          <div class="flex justify-center items-center gap-4 mt-10">
            <button class="btn-ghost micro-lift px-3 py-1.5 text-sm text-muted" ?disabled=${this.page <= 1} @click=${() => this._go(this.page - 1)}>�?上一�?/button>
            <span class="text-sm text-subtle font-mono">${this.page} / ${this._pages}</span>
            <button class="btn-ghost micro-lift px-3 py-1.5 text-sm text-muted" ?disabled=${this.page >= this._pages} @click=${() => this._go(this.page + 1)}>下一�?�?/button>
          </div>
        ` : nothing}
        ...
        ${this.confirmSlug ? html`
          <div class="fixed inset-0 z-50 flex items-center justify-center" @click=${(e: Event) => { if (e.target === e.currentTarget) this.confirmSlug = ''; }}>
            ...
            <div class="card micro-lift p-6 max-w-sm w-full mx-4 modal-enter relative z-10">
              <p class="text-fg font-medium mb-2">确认删除</p>
              <p class="text-sm text-muted mb-6">此操作不可撤销</p>
              <div class="flex justify-end gap-3">
                <button class="btn-ghost micro-lift px-4 py-2 text-sm text-muted" @click=${() => this.confirmSlug = ''}>取消</button>
                <button class="btn-primary micro-lift px-4 py-2 text-sm" style="background:rgb(239 68 68)" @click=${this._confirmDelete}>删除</button>
              </div>
            </div>
          </div>
        ` : nothing}
