...
| 日期 | 更新 |
|------|------|
| 2026-07-22 | UI/UX 重构：编辑式极简方向，移除卡片阴影改为分隔线列表，精致 expo-out 缓动替代 spring bounce，微光骨架屏，背景氛围渐变，下划线式导航激活态 |
| 2026-07-22 | 搜索关键词高亮（post-card summary 匹配）+ 自定义删除弹窗 |
| 2026-07-22 | 自定义删除确认弹窗（替换 window.confirm）+ 编辑器预览切换 |
| 2026-07-21 | 编辑器 Markdown 预览 + 后台总数显示 |
| 2026-07-21 | token 验证接入前台 + docker 密码强制 env + rand.Read 错误检查 |
| 2026-07-21 | CORS 精简 + Search 强类型 + Admin 分页（size=20） |
| 2026-07-21 | 中文 slug 修复（时间戳兜底）+ 前端 token 有效性验证 |
